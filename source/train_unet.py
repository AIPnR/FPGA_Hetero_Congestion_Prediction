...
def dataset_reformulate(dataset):
    dataset_grid_feature = [g_emb2image((data[0].nodes['grid'].data['feature'])[:,[0,4]]) for data in dataset]
...
def train_fn(
    gen, loader, opt_gen, fn_loss, g_scaler, epoch, scheduler_gen
):
    time_begin = time()
...
def main():
    ...
    gen = Generator(in_channels=2, out_channels=4, features=16, image_size=args.cnn_image_size, decoder = True).to(DEVICE)
    # gen = GEN(gen_ori)
    ...
    for epoch in range(args.num_epoch):
        ...
        train_time = train_fn(gen, train_dataset, opt_gen, FN_LOSS, g_scaler, epoch, scheduler_gen)
        ...
        if args.save_model and ((epoch+1)%args.save_model_freq ==0):
            save_model_checkpoint(gen, opt_gen, epoch+1, CHECKPOINT_MODEL_DIR)
