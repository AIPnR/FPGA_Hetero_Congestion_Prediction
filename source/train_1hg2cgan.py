...
class PREDICTOR_DISC(nn.Module):
    ...
    def forward(self, graph, chan_usage_cut_image):
        grid_feature_emb = graph.nodes['grid'].data['feature']
        grid_emb_image = g_emb2image(grid_feature_emb)
        d_out = self.disc(grid_emb_image, chan_usage_cut_image)
        return d_out
...
def main():
    ...
    disc_base = Discriminator(channels_xny=in_grid_feats + args.nn_out_dim, 
                         features=args.hidden_num, 
                         image_size=args.cnn_image_size).to(DEVICE)
    
    disc = PREDICTOR_DISC(disc_base)
